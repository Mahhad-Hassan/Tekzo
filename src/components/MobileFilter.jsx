...
const MobileFilter = ({openFilter,setOpenFilter,search,setSearch,brand,setBrand,priceRange,setPriceRange,category,setCategory,handleCategoryChange,handleBrandChange}) => {
      ...
      <div className='flex flex-col gap-2 mt-3'>
        {
          categoryOnlyData?.map((item,index)=>{
            ...
            <select name="" id="" className='w-full bg-white border-gray-200 border-2 rounded-md cursor-pointer' 
            value={brand}
            onChange={handleBrandChange}
            >
              {
                brandOnlyData?.map((item,index)=>{
                  return   <option   key={index}  value={item ? item.toUpperCase() : ""} className="w-full" > 
                    {item}
                  </option>
                })
              }
